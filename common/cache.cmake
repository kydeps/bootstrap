...
#
# * if cache is disabled (KYDEPS_CACHE=OFF) *or* if the packages is already
#   installed under /i -> do nothing
#
# // if cache is enabled
#
# * if a zip of the package is not found in /c (local miss) - if a remote URL is
#   set -> try to download zip (remote hit/miss)
#
# * if a zip of the package is found in /c (local or remote hit) -> unpack it in
#   /i
#
...
function(CacheUpdate KYDEP)
    ...
    set(C_PATH "${KYDEPS_BINARY_DIR}/c/${_KEY}.zip")

    if(KYDEPS_CACHE)
        if("-${I_CONTENTS}-" STREQUAL "--")
            AddContext("empty::skipped")
            file(REMOVE "${C_PATH}")
        elseif(EXISTS "${C_PATH}")
            AddContext("exists::skipped")
        else()
            AddContext("updated")
